...
for model in refol fedostc; do
  echo "MODEL_START $model $(date -Is)" | tee -a "$RESULT_DIR/master.log"
  ...
  status=$?
  echo "MODEL_DONE $model status=$status $(date -Is)" | tee -a "$RESULT_DIR/master.log"
  if [ "$status" -ne 0 ]; then
    echo "STOP_AFTER_FAILURE $model" | tee -a "$RESULT_DIR/master.log"
    exit "$status"
  fi
done
.venv/bin/python - <<'PY' "$RESULT_DIR"
import json, sys
...
PY
echo "ALL_DONE $(date -Is)" | tee -a "$RESULT_DIR/master.log"
